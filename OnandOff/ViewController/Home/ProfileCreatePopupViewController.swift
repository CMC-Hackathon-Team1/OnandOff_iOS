//
//  ProfileCreatePopupViewController.swift
//  OnandOff
//
...
//페르소나 만들기 경고창 (아직 미구현)
class ProfileCreatePopupViewController: UIViewController{
    //MARK: - Datasource
    ...
    let popupLabel = UILabel().then{
        ...
        $0.text = "페르소나는 한 번 생성되면 더이상 수정할 수 없습니다. \n이대로 생성하시겠습니까? \n(페르소나 변경을 원할 시, 프로필을 다시 만들어야합니다.)"
        $0.numberOfLines = 0
        $0.textAlignment = .center
    }
    let createView = UIView().then{
        $0.backgroundColor = .mainColor
    }
    let create = UILabel().then{
        $0.text = "생 성"
        $0.textColor = .white
    ...
    func setUpView(){
        ...
        popupView.addSubview(createView)
    ...
    @objc func didClickCreate(sender: UITapGestureRecognizer){
    ...
    func layout(){
        ...
        self.createView.snp.makeConstraints{
            $0.trailing.bottom.equalToSuperview()
            $0.width.equalToSuperview().dividedBy(2)
            $0.height.equalTo(38)
        }
        self.create.snp.makeConstraints{
            $0.centerX.equalToSuperview()
            $0.centerY.equalToSuperview()
        }
    ...
    func addTarget(){
        ...
        let createBtn = UITapGestureRecognizer(target: self, action: #selector(didClickCreate))
        createView.isUserInteractionEnabled = true
        createView.addGestureRecognizer(createBtn)
